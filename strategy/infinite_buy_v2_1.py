                ...
                # if (self.is_ma_cut and row["Macd"] < row["MacdSignal"]):
                    ...
                    if self.is_quarter_mode and (position["AvailableBuyCash"] - position["TotalBuyPrice"]) < position["OneBuyCash"] :
                        ...
                        # 후반전이면
                            ...
                            # 1/4를 평단가에 매도
                            target_price1 = position["EntryPrice"] # + 수수료 고려하여 위에서 팔 경우 코딩 수정 필요
                            target_shares1 = math.floor(shares / 4)
                            ...
                            # 나머지 2/4를 평단가 +10%에 지정가 매도
                            target_price3 = position["EntryPrice"] * 1.1 # + 수수료 고려하여 위에서 팔 경우 코딩 수정 필요
                            target_shares3 = shares - target_shares1 - target_shares2

                            if target_shares3 > 0 and row["High"] > target_price3:
                                amount = target_price3 * target_shares3
                                sell_commission = math.floor(amount * self.commission * 100) / 100
                                cash += amount - sell_commission  # 주식 판매로 얻은 금액 추가
                                position["Shares"] -= target_shares3
                                position["TotalBuyPrice"] = position["EntryPrice"] * position["Shares"]

                                # 손익금
                                profit = amount - (position["EntryPrice"] * target_shares3) - sell_commission
                                position["Profit"] += profit
